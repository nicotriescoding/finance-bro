...
Extract every task, the official answer if the solution sheet is included, and
the exam identifier (term + task number). If the correct answer is
...
3. **Change every number.** The seeded `build` does this for free: choose
   draw ranges so the exam's own parameter set cannot come out.
...
Every task becomes `kind: "numeric"` with a seeded `build` (banks are
numeric-only by decision 2026-08-28) - that turns one exam question into
unlimited practice. A multiple-choice exam task is redesigned around the
calculation it tests. If a task is purely conceptual and cannot be made
numeric, skip it and list it as skipped in the report. Write every formula as
...
- anything skipped (exhibits, purely conceptual tasks), and why
