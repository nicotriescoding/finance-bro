...
| 31 | **Finance-bro field guide** (2026-09-14): `/what-is-a-finance-bro` - one long article defining the finance bro (habitat, uniform, glossary, diet, dating, life cycle) plus the two sibling archetypes BWL Marie and Jura Justus, a field-research link to the creator The Swiss Chris (link only - no hosted creator photo, § 22 KUG), two CC0 photos in `public/bro/` (Burst model-released stock + a faceless Commons shot), Article + FAQPage JSON-LD single-sourced from `src/content/finance-bro.ts`; linked from the footer line + sitemap (0.7), TUM-free metadata, 11 smoke checks | done |
