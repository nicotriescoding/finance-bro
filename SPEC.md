...
| 16 | `/products` page v2 "the bundles": eight joke bundles since 2026-09-06 (FinanceBro Starter Pack 6 - Birkin card shows a real orange ostrich Birkin, BWL Marie 7, Undercover Broke Student 6, Excel Monkey 4, Doomsday Bunker 4, LinkedIn Thought Leader 4, Boring Index Fund 4, After-Exam Party Kit 5: Aperol tower, beer pong, shot roulette, spritz glasses, party speaker - beer mortar opener dropped 2026-09-08) in a 2-col card grid; images are Adobe Stock (free tier, licensed on Nico's account, committed under `public/products/`, scouted 5+ candidates each) plus four Wikimedia Commons CC BY-SA photos (Birkin, Red Bull can, JBL PartyBox, beer tower) credited in the footer, object-contain so nothing crops; every product links one specific amazon.de listing (ASIN via `amzProduct`, no search links; tag pending), calculator card = Casio FX-85MS (TUM calculator policy), Birkin sold-out gag, compact § 5a UWG disclosure; ketchup + cigarettes removed 2026-08-29 per Nico | done - tag TODO in `src/lib/affiliate.ts` |
| 17 | Library (`/library`) v2 "portfolio statement": 2-col book grid, fund-overview chips (positions/avg ROI/top holding), 8 read books with self-hosted covers under `public/covers/` (sourced once from Open Library, Carnegie fixed to the English edition; never hotlinked), ROI-multiplier ratings (Lean Startup ×67 per Nico), small print merged into one compact card; ad-free by rule | done |
...
| 19 | Consent-gated PostHog (EU cloud): token baked into `src/lib/analytics.ts`; Google's TCF consent dialog is the cookie banner, the site's own banner is a fallback that only appears when Google's dialog never shows | live since 2026-09-06 |
