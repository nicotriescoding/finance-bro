...
| 29 | **Search metadata, scenario A** (2026-09-11): `src/lib/seo.ts` - per-route canonical + Open Graph via `pageMeta()`, geo terms (München, Garching, Arcisstraße, Straubing, Heilbronn, Ottobrunn) and German search terms in the root/subject descriptions + meta keywords, JSON-LD `@graph` (Organization with `areaServed`, WebSite, WebApplication with EducationalAudience); TUM named in metadata only on `/`, `/career`, `/quiz?subject=` (smoke-guarded). Scenarios B (bottom-of-page geo line) and C (static subject routes + a BWL München entrance page) undecided - see BACKLOG | done |
